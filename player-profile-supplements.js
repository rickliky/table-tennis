...
fetch('./player-profile-supplements.json').then(response => response.ok ? response.json() : Promise.reject()).then(data => {
  ...
  list.innerHTML = [...groups.entries()].filter(([, entries]) => entries.length).map(([category, entries]) => `<section><h2>🏆 ${category}</h2><div>${entries.sort((left, right) => left.kanjiName.localeCompare(right.kanjiName, 'ja')).map(entry => `<article><header><h3>${entry.kanjiName}</h3><span>${entry.romanizedName}</span></header><dl><div><dt>利き手 / Hand</dt><dd>${entry.playingHand === '右' ? '右 / Right' : '左 / Left'}</dd></div>${entry.grip ? `<div><dt>グリップ / Grip</dt><dd>${entry.grip}</dd></div>` : ''}<div><dt>戦型 / Style</dt><dd>${entry.playingStyle}</dd></div><div><dt>フォア / Forehand</dt><dd>${entry.forehandType} · ${entry.forehandModel}</dd></div><div><dt>バック / Backhand</dt><dd>${entry.backhandType} · ${entry.backhandModel}</dd></div></dl></article>`).join('')}</div></section>`).join('') || '<p class="loading">登録済みの入力はありません。 / No submissions yet.</p>';
