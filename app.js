...
function renderYearlySummary() {
  ...
  document.querySelector('#yearly-summary').innerHTML = `<article><header><p>${selectedSummaryYear || '-'}</p><strong>${matches.length}<small>${t('matches')}</small></strong><span>${participants.size} ${t('players')} ${t('participated')}</span></header><div class="monthly-leader"><b>${t('yearLeader')}</b><h3>${rankings[0] ? nameFor(rankings[0].player) : '-'}</h3><span>${rankings[0] ? `${Math.round(rankings[0].stats.wins / (rankings[0].stats.wins + rankings[0].stats.losses) * 100)}% · ${rankings[0].stats.wins + rankings[0].stats.losses} ${t('gamesPlayed')}` : '-'}</span></div><div class="podiums">${podiums}</div></article>`;
}
