...
const eligibility = (matches, matchesPerDay = 1) => { const gameDays = new Set(matches.filter(isComplete).map(match => match.matchDate)).size, minimum = gameDays * matchesPerDay; return { minimum, gameDays, text: language === 'en' ? `Eligibility: ${matchesPerDay} completed match${matchesPerDay === 1 ? '' : 'es'} for every match day in the selected period.` : `対象: 選択期間内のすべての試合日につき${matchesPerDay}試合。` }; };
...
function renderThreeMonthSummary() {
  ...
  recentStart = Math.min(recentStart, Math.max(0, allMonths.length - 2));
  const months = allMonths.slice(recentStart, recentStart + 2);
  document.querySelector('#recent-prev').disabled = recentStart >= allMonths.length - 2;
  document.querySelector('#recent-next').disabled = recentStart === 0;
  monthlyCategoryEntries = new Map(); monthlyCategoryMatches = new Map(); const report = months.map(month => {
    ...
    const periodEligibility = eligibility(matches, 2), rankings = rankPlayers(matches).filter(entry => entry.stats.wins + entry.stats.losses >= periodEligibility.minimum);
    const groups = new Map();
    ...
    categories.forEach(([category, entries]) => { monthlyCategoryEntries.set(`${month}|${category}`, entries); monthlyCategoryMatches.set(`${month}|${category}`, matches); }); const leaders = categories.map(([category, entries]) => `<section class="category-podium"><button class="category-ranking-open" data-month="${month}" data-category="${category}">🏆 ${category}<small>${language === 'en' ? 'View all rankings' : '全順位を見る'}</small></button>${entries.slice(0, 3).map((entry, index) => `<div><i>${rankIcon(index)}</i><b>${playerLink(entry.player)}</b>${recordSummary(entry.stats, matches, entry.player.playerId)}</div>`).join('')}</section>`).join('');
...
function renderYearlySummary() {
  const years = [...new Set(data.matches.filter(match => eventType(match) === 'training' && match.matchDate).map(match => match.matchDate.slice(0,4)))].sort().reverse(); leaderboardYearIndex = Math.min(leaderboardYearIndex, Math.max(0, years.length - 1)); const year = years[leaderboardYearIndex] || '', matches = data.matches.filter(match => eventType(match) === 'training' && match.matchDate.startsWith(year)), rankings = rankPlayers(matches).filter(entry => entry.stats.wins + entry.stats.losses >= eligibility(matches).minimum);
  ...
  categoryLeaderboardEntries = new Map(categoryEntries); categoryLeaderboardYear = year; categoryLeaderboardMatches = matches; const participants = new Set(matches.filter(isComplete).flatMap(match => [match.player1Id,match.player2Id])).size, periodEligibility = eligibility(matches), winLabel = language === 'ja' ? '勝' : 'W', lossLabel = language === 'ja' ? '負' : 'L'; document.querySelector('#overall-link-title').textContent = language === 'en' ? `${year} Yearly Leaderboard` : `${year}年リーダーボード`; document.querySelector('#overall-year-prev').disabled = leaderboardYearIndex >= years.length - 1; document.querySelector('#overall-year-next').disabled = leaderboardYearIndex === 0; document.querySelector('#overall-link-detail').textContent = language === 'en' ? `Top 3 by category · ${participants} players · ${periodEligibility.text}` : `カテゴリ別 TOP 3 · ${participants}人 · ${periodEligibility.text}`; document.querySelector('#overall-stat-list').innerHTML = categoryEntries.map(([category, entries]) => `<section><button class="category-ranking-open" data-category="${category}">🏆 ${category}<small>${language === 'en' ? 'View all rankings' : '全順位を見る'}</small></button>${entries.slice(0,3).map((entry,index) => { const games = entry.stats.wins + entry.stats.losses, rate = Math.round(entry.stats.wins / games * 100), categories = opponentCategoryBreakdown(matches, entry.player.playerId); return `<span><i>${rankIcon(index)}</i><a class="player-link" href="player.html?id=${entry.player.playerId}">${nameFor(entry.player)}<small class="alltime-record"><b class="${rate >= 50 ? 'positive' : 'negative'}" title="${categories}">${rate}%</b><em><i class="win">${winLabel}</i>${entry.stats.wins} <i class="loss">${lossLabel}</i>${entry.stats.losses} · ${language === 'en' ? `${games}G` : `${games}試合`}</em><em class="leader-category-records">${categories.replace(/\n/g,' · ')}</em></small></a></span>`; }).join('')}</section>`).join('');
}
...
document.querySelector('#overall-year-prev').onclick = () => { leaderboardYearIndex++; renderYearlySummary(); };
document.querySelector('#overall-year-next').onclick = () => { leaderboardYearIndex--; renderYearlySummary(); };
const howToRead = {
