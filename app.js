...
const resultDots = (matches, playerId) => { const completed = matches.filter(isComplete), wins = completed.filter(match => match.winnerId === playerId).length, losses = completed.length - wins, winLabel = language === 'ja' ? '勝' : 'W', lossLabel = language === 'ja' ? '負' : 'L'; return `<aside class="match-result-dots"><i class="win">${winLabel}</i><b>${wins}</b><i class="loss">${lossLabel}</i><b>${losses}</b></aside>`; };
