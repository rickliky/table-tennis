...
const resultDots = (matches, playerId) => `<aside class="match-result-dots">${matches.filter(isComplete).map(match => { const win = match.winnerId === playerId; return `<i class="${win ? 'win' : 'loss'}">${language === 'ja' ? (win ? '勝' : '負') : (win ? 'W' : 'L')}</i>`; }).join('')}</aside>`;
