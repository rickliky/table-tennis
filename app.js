...
function renderYearlySummary() {
  ...
  const participants = new Set(matches.filter(isComplete).flatMap(match => [match.player1Id,match.player2Id])).size, winLabel = language === 'ja' ? '勝' : 'W', lossLabel = language === 'ja' ? '負' : 'L'; document.querySelector('#overall-link-detail').textContent = language === 'en' ? `Top 3 by category · ${participants} players` : `カテゴリ別 TOP 3 · ${participants}人`; document.querySelector('#overall-stat-list').innerHTML = categoryEntries.map(([category, entries]) => `<section><b>🏆 ${category}</b>${entries.slice(0,3).map((entry,index) => { const games = entry.stats.wins + entry.stats.losses, rate = Math.round(entry.stats.wins / games * 100); return `<span><i>${rankIcon(index)}</i><a class="player-link" href="player.html?id=${entry.player.playerId}">${nameFor(entry.player)}<small class="alltime-record"><b class="${rate >= 50 ? 'positive' : 'negative'}" title="${opponentCategoryBreakdown(matches, entry.player.playerId)}">${rate}%</b><em><i class="win">${winLabel}</i>${entry.stats.wins} <i class="loss">${lossLabel}</i>${entry.stats.losses} · ${games}G</em></small></a></span>`; }).join('')}</section>`).join('');
}
