...
let data = { players: [], matches: [] }, selectedMonth = 'all', selectedYear = 'all', selectedSession = '', calendarMonth = '', recentStart = 0, leaderboardYearIndex = 0, categoryLeaderboardEntries = new Map(), categoryLeaderboardYear = '', monthlyCategoryEntries = new Map(), charts = [];
...
const eligibility = matches => { const gameDays = new Set(matches.filter(isComplete).map(match => match.matchDate)).size, minimum = gameDays; return { minimum, gameDays, text: language === 'en' ? 'Eligibility: 1 match for every match day in the selected period.' : '対象: 選択期間内のすべての試合日につき1試合。' }; };
...
function renderThreeMonthSummary() {
  ...
  monthlyCategoryEntries = new Map(); const report = months.map(month => {
