    ...
    if (url.pathname === '/api/approve' && request.method === 'POST') {
      ...
      return json(await repo.withLock(environment, async () => {
        ...
        if (body.decision === 'reject') {
          await repo.write(environment, 'pending-changes', changes.map(item => item.changeId === change.changeId ? { ...item, status: 'rejected', reviewedAt: new Date().toISOString(), reviewedBy: actor.role } : item));
          return { ok: true };
        }
        const collection = collectionFor(change.entityType);
        const current = await repo.read(environment, collection);
        const idField = { club: 'clubId', player: 'playerId', match: 'matchId', externalOpponent: 'externalOpponentId', tournament: 'tournamentId', tournamentMatch: 'tournamentMatchId' }[change.entityType];
        if (change.action !== 'create' && change.before && JSON.stringify(current.find(item => item[idField] === change.targetId)) !== JSON.stringify(change.before)) throw new Error('Change is stale and must be resubmitted');
